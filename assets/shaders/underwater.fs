...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords ) {
    ...
    vec2 p = uv * 5.0;
    float t = underwater.y * 1.5;
    float w1 = sin(p.x + t) * cos(p.y - t);
    float w2 = sin(p.x * 0.8 - t * 0.5) * cos(p.y * 1.2 + t * 0.7);
    float caustics = pow(abs(w1 + w2) * 0.5, 2.0); 
    
    vec4 final_color = tex;
    final_color.rgb = mix(final_color.rgb, vec3(0.0, 0.3, 0.6), 0.4 * tex.a);
    final_color.rgb += vec3(0.2, 0.8, 1.0) * caustics * 1.5 * tex.a;
    
    final_color.a += (underwater.x + underwater.y + time) * 0.00000001; 
