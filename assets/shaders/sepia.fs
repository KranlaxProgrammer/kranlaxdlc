...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords ) {
    ...
    // Filtro base Sepia
    float r = dot(tex.rgb, vec3(0.393, 0.769, 0.189));
    float g = dot(tex.rgb, vec3(0.349, 0.686, 0.168));
    float b = dot(tex.rgb, vec3(0.272, 0.534, 0.131));
    vec3 col = vec3(r, g, b);
    
    // Reloj falso a 12 FPS para el efecto de cinta vieja
    float t = floor(sepia.y * 12.0); 
    ...
    float scratch_pos = fract(sin(t * 12.345) * 43758.5453);
    float scratch_dist = abs(uv.x - scratch_pos);
    if (scratch_dist < 0.005 && fract(sin(uv.y * 100.0 + t)) > 0.5) {
        col -= 0.3; 
    }
    
    // Polvo / Pelusas blancas y negras
    float dust = fract(sin(dot(uv + t * 1.5, vec2(53.123, 12.345))) * 43758.5453);
    if (dust > 0.995) {
        col += 0.4;
    } else if (dust < 0.005) {
        col -= 0.4;
    ...
    float vignette = 1.0 - dot(center_uv, center_uv) * 0.25;
    col *= vignette;
    
    // Parpadeo de luz del proyector
    float flicker = 0.9 + 0.1 * fract(sin(t * 7.123) * 43758.5453);
    col *= flicker;
    
    vec4 final_color = vec4(col, tex.a);
    
    // ESCUDO ANTI-CRASH TOTAL
    final_color.a += (sepia.x + sepia.y + time) * 0.00000001; 
