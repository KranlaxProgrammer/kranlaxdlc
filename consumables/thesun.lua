SMODS.Consumable {
    ...
    -- AÑADIDO: Tooltip del Sello Naranja con variables "???"
    loc_vars = function(self, info_queue, card)
        info_queue[#info_queue+1] = {set = 'Other', key = 'kranlaxs_orangeseal_seal', vars = {'???', '???'}}
        return {vars = {}}
    end,
            ...
            G.E_MANAGER:add_event(Event({
                ...
                func = function()
                    for _, c in ipairs(G.hand.highlighted) do
                        c:set_seal('kranlaxs_orangeseal', true, true)
                    end
                    return true
                end
            }))
