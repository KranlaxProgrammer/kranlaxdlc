SMODS.Consumable {
        ...
        if target_card then
            ...
            G.E_MANAGER:add_event(Event({
                ...
                func = function()
                    ...
                    -- Edición Laminado (Foil)
                    target_card:set_edition({foil = true}, true, true)
                    return true
                end
            }))
        end
    ...
    check_for_unlock = function(self, args)
        if args.type == 'win' then
            -- Comprueba que la baraja sea la de Plasma ('b_plasma') y el pozo sea tu Pozo de Platino
            if G.GAME.selected_back.effect.center.key == 'b_plasma' and G.GAME.stake == 'kranlaxs_platinum' then
