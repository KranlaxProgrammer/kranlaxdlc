SMODS.Joker{ --Epic beanstalk
    ...
    calculate = function(self, card, context)
                ...
                extra = {
                    func = function()
                        ...
                        G.E_MANAGER:add_event(Event({
                            func = function()
                                local joker_card = SMODS.add_card({ set = 'Joker', key = 'j_kranlaxs_magnificentseed' })
                                return true
                            end
                        }))
        ...
        if context.end_of_round and context.game_over == false and context.main_eval  and not context.blueprint then
            if to_big((card.ability.extra.conteo or 0)) == to_big(3) then
                return {
                    func = function()
                        local created_joker = true
                        G.E_MANAGER:add_event(Event({
                            func = function()
                                local joker_card = SMODS.add_card({ set = 'Joker', key = 'j_kranlaxs_zenithbeanstalk' })
                                return true
                            end
                        }))
                        if created_joker then
